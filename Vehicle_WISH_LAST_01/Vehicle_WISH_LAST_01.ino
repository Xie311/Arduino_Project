...
if (num<=2&&num>0){
    ...
    {
        setDifferentialDrive(10,16);//01111只剩最左边压线了，要最大量的往左边转
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && (r[5] == BLACK || r[6] == BLACK) && r[7] != BLACK)
    {
        setDifferentialDrive(12,10);//
    }
...
}else if (num>=3&&num<=4)
{
    if ((r[0] == BLACK || r[1] == BLACK || r[2] == BLACK || r[3] == BLACK ) && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(10,-16);//右边大转
        //flag=1;
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && (r[4] == BLACK ||r[5] == BLACK || r[6] == BLACK|| r[7] == BLACK))
    {
        setDifferentialDrive(10,16);//
        //flag=2;
    }  else if (r[0] == BLACK && r[1] == BLACK && r[2] != BLACK && r[3] != BLACK && r[4] == BLACK &&r[5] != BLACK && r[6] != BLACK&& r[7] != BLACK)
    {
        setDifferentialDrive(10,16);//
...
else if (num==7)
{
    if (r[7] != BLACK)
    {
        setDifferentialDrive(6,-16);//右边大转
        flag=1;
    }
    else if (r[0] != BLACK)
    {
        setDifferentialDrive(6,16);//
        flag=2;
    }else
    {
        //setDifferentialDrive(0,8);//直行
