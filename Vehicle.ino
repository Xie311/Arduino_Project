...
#define PWML 9  //用于电机1的PWM 输出，调节电机速度 
#define INL1 12   
//11 和 12 引脚用于电机1的转动方向控制 
#define INL2 11    
...
#define WHEEL_BASE 20.0f      // 两轮间距（cm）
#define BLACK 0
#define LIGHT 1
...
if (r[0] == BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(100,-30);//01111只剩最右边压线了，要最大量的往右边转
    }
    else if (r[0] != BLACK && (r[1] == BLACK || r[2] == BLACK )&& r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(100,-15);//
    }
    else if (r[0] != BLACK && r[1]! = BLACK && r[2] != BLACK && (r[3] == BLACK || r[4] == BLACK) && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(100,0);//直行
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] == BLACK)
    {
        setDifferentialDrive(100,30);//01111只剩最左边压线了，要最大量的往左边转
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] == BLACK && r[3] != BLACK && r[4] != BLACK && (r[5] == BLACK || r[6] == BLACK) && r[7] != BLACK)
    {
        setDifferentialDrive(100,-15);//
