...
#define ENCODER_B_R  10 
#define PWML 11  //用于电机1的PWM 输出,调节电机速度  
#define PWMR 14  //用于电机2的PWM输出,调节电机速度 
#define DIRL 6   //6 和7引脚用于电机的转动方向控制 
#define DIRR 7    

#define PERIOD 14
#define WHEEL_RADIUS 10.0f    // 轮子半径（cm）
...
int flag=0;
int r[8];
volatile float TARGET_L = 14;  //定义电机 1 的目标速度值 
volatile float TARGET_R = 14;  //定义电机 2 的目标速度值 
...
float kp = 8, TI =100 , TD =110,T = PERIOD; 
...
void control_L() 
{ 
velocity_L = (encoderVal_L/780) * 3.14110 * 2.0 * (1000/PERIOD);   
...
void control_R() 
{ 
velocity_R = (encoderVal_R/780) * 3.14110 * 2.0 * (1000/PERIOD);   
...
angle3=1100;    // 90度嵌入舵机座,角度变大,向上抬升
myservo3.write(angle3);   
delay(800);
angle1 = 0; //0~180  
myservo1.write(angle1);  
...
if (num<=2&&num>0){
    ...
    {
        setDifferentialDrive(10,-10);//01111只剩最右边压线了，要最大量的往右边转
        delay(200);
    }
    else if (r[0] != BLACK && (r[1] == BLACK || r[2] == BLACK )&& r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(16,-12);//右边转  maybe12
    }
    else if (r[0] != BLACK && r[1]!= BLACK && r[2] != BLACK && (r[3] == BLACK || r[4] == BLACK) && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(16,0);//直行
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] == BLACK)
    {
        setDifferentialDrive(10,10);//01111只剩最左边压线了，要最大量的往左边转
        delay(200);
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && (r[5] == BLACK || r[6] == BLACK) && r[7] != BLACK)
    {
        setDifferentialDrive(16,12);//
    }
    else//到这里说明黑色部分以一种比较诡异的方式在分布，反正不是正常循迹，慢速前行
    {
        setDifferentialDrive(12,0);//直行
    }
}
else if (num>=3&&num<=4)
{
    if ((r[0] == BLACK || r[1] == BLACK || r[2] == BLACK || r[3] == BLACK ) && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(10,-10);//右边大转
        delay(200);
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && (r[4] == BLACK ||r[5] == BLACK || r[6] == BLACK|| r[7] == BLACK))
    {
        setDifferentialDrive(10,10);//
        delay(200);
    }
}else if (num>=5&&num<=6)
{
