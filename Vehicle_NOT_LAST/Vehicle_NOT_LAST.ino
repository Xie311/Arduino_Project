...
volatile PID_t pidLP,pidRP,pidLV,pidRV;
volatile float encoderposR,encoderposL;
volatile int flag=0;
int r[8];
...
void positionServo(float ref,int motor)
{
if(motor==1)
{
    pidLP.ref = ref;
    pidLP.fdb = encoderposL/780 * 3.1415 * 2.0;
    PID_Calc_P(&pidLP);
    
    pidLV.ref = pidLP.output;
    pidLV.fdb =velocity_L;
    PID_Calc_P(&pidLV);
}
if(motor==2)
{
    pidRP.ref = ref;
    pidRP.fdb = encoderposL/780 * 3.1415 * 2.0;
    PID_Calc_P(&pidRP);
    
    pidRV.ref = pidRP.output ;
    pidRV.fdb = velocity_R ;
    PID_Calc_P(&pidRV);
}
}
void speedServo(float ref,int motor)
{
    if(motor==1)
{
    pidLV.ref = ref;
...
}else if (num>=3&&num<=4)
{
    if ((r[0] == BLACK || r[1] == BLACK || r[2] == BLACK || r[3] == BLACK ) && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(6,-12);//右边大转
        //flag=1;
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && (r[4] == BLACK ||r[5] == BLACK || r[6] == BLACK|| r[7] == BLACK))
    {
        setDifferentialDrive(6,12);//
        //flag=2;
    }else
    {
        setDifferentialDrive(6,0);//直行
...
else if (num==7)
{
    if (r[7] != BLACK)
    {
        setDifferentialDrive(3,-12);//右边大转
    }
    else if (r[0] != BLACK)
    {
        setDifferentialDrive(3,12);//
    }else
    {
        setDifferentialDrive(6,0);//直行
    }
