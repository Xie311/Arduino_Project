...
#define PIN_SERVO2 9
#define PIN_SERVO3 8  
#define REC_TIME   200  //500 直角延迟时间
#define REC_W      15  //15  直角角速度
#define SPEED_STR  10   //直行速度
#define SPEED_3 5    //L3，R3前进速度
#define SPEED_2 5    //L2,R2前进速度
#define REC_STR 1     //直角处前进速度
#define W_3 4        //L3，R3角速度
#define W_2 2         //L2，R2角速度
...
void loop() { 
unsigned long convertedTime = millis() - timestamp; //经过的时间
float seconds = convertedTime / 1000.0;
//判断红外的情况
r[0]=analogRead(rIr0);
r[1]=analogRead(rIr1);
r[2]=analogRead(rIr2);
...
//     if(flag==1&&seconds>35){
//         setDifferentialDrive(REVERSE_SPEED,2);
//         delay(200);
//     }
// }
if (num<=2&&num>0){
    if (r[0] == BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
         ...
         setDifferentialDrive(REC_STR,-REC_W);//01111只剩最右边压线了，要最大量的往右边转
         delay(REC_TIME);
         flag=1;

        
    }
    else if (r[0] != BLACK && (r[1] == BLACK )&& r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_3,-W_3);//右边转  maybe12
        flag=0;
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] == BLACK  && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_2,-W_2);//右边转  maybe12
        flag=0;
    }
    else if (r[0] != BLACK && r[1]!= BLACK && r[2] != BLACK && (r[3] == BLACK || r[4] == BLACK) && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_STR,0);//直行
        flag=0;
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] == BLACK)
    {
         setDifferentialDrive(REC_STR,REC_W);//01111只剩最左边压线了，要最大量的往左边转
         delay(REC_TIME);
         flag=1;

    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK && r[4] != BLACK &&  r[6] == BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_3,W_3);//
        flag=0;
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[5] == BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_2,W_2);//
        flag=0;
    }
...
{
    if (r[0] == BLACK && r[1] == BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
         setDifferentialDrive(REC_STR,-REC_W);//右边大转
         delay(REC_TIME);
         flag=1;

    }
    else if (r[0] != BLACK && r[1] == BLACK && r[4] != BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_3,-W_3);//右边大转
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] == BLACK && r[5] != BLACK && r[6] != BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_2,-W_2);//右边大转
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK &&  r[6] == BLACK && r[7] == BLACK)
    {
         setDifferentialDrive(REC_STR,REC_W);//
         delay(REC_TIME);
         flag=1;

    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[3] != BLACK &&  r[6] == BLACK && r[7] != BLACK)
    {
        setDifferentialDrive(SPEED_3,W_3);//
    }
    else if (r[0] != BLACK && r[1] != BLACK && r[2] != BLACK && r[5] == BLACK &&  r[6] != BLACK && r[7] != BLACK)
    {
        ...
        setDifferentialDrive(SPEED_STR,0);
    }
    //flag=0;
}else if (num>=5&&num<=6)
{
    if (r[0] == BLACK && r[1] == BLACK)
    {
        setDifferentialDrive(REC_STR,-REC_W);//右边大转
        delay(REC_TIME);
    }
    else if (r[6] == BLACK && r[7] == BLACK)
    {
        setDifferentialDrive(REC_STR,REC_W);//
        delay(REC_TIME);
    }
